...
public class StatusPane extends Component {
	...
	// potentially adjusts the row(s) of the the buff indicator to avoid some
	// cutouts
	...
	public StatusPane(boolean large) {
		...
		if (large)
			bg = new NinePatch(frameAsset, 0, 64, 41, 39, 33, 0, 4, 0);
		else
			bg = new NinePatch(frameAsset, 0, 0, 82, 38, 32, 0, 5, 0);
		add(bg);
		...
		if (large)
			shieldHP = new Image(barAsset, 0, 112, 128, 9);
		else
			shieldHP = new Image(barAsset, 0, 44, 50, 4);
		add(shieldHP);

		if (large)
			hp = new Image(barAsset, 0, 103, 128, 9);
		else
			hp = new Image(barAsset, 0, 40, 50, 4);
		add(hp);
		...
		if (large)
			exp = new Image(barAsset, 0, 121, 128, 7);
		else
			exp = new Image(barAsset, 0, 48, 17, 4);
		add(exp);
	...
	protected void layout() {
		...
		bg.y = y;
		if (large)
			bg.size(160, bg.height); // HP bars must be 128px wide atm
		else
			bg.size(hpBarMaxWidth + 32, bg.height); // default max right is 50px health bar + 32
		...
		if (large) {
			...
			if (hpBarMaxWidth < 82) {
				// the class variable assumes the left of the bar can't move, but we can inset
				// it 9px
