...
public class AboutScene extends PixelScene {
	...
	public void create() {
		...
		if (landscape()) {
			...
			iatdb.setRect((w - fullWidth) / 2f, insets.top + 6, 120, 0);
		}
		content.add(iatdb);
		...
		shpx.setRect(iatdb.left(), iatdb.bottom() + 8, colWidth, 0);
		content.add(shpx);
		...
		if (landscape()) {
			alex.setPos(shpx.right(), Math.max(shpx.top(),
					shpx.top() + (shpx.height() - alex.height() * 2) / 2f));
		} else {
