...
public class GameScene extends PixelScene {
	...
	public void create() {
		...
		// display cutouts can obstruct various UI elements, so we need to adjust for
		// that sometimes
		...
		if (largeInsetTop == 0 && insets.top > 0) {
			// smaller non-notch cutouts are of varying size and may obstruct various UI
			// elements
			...
			if (cutout.left < 84
				...
				int i = 1;
				int rowTop = 11;
				// in most cases this just obstructs one row, but dynamic island can block more
				// =S
		...
		status.setRect(insets.left, uiSize > 0 ? uiCamera.height - 39 - insets.bottom : screentop,
				uiCamera.width - insets.left - insets.right, 0);
		add(status);
		...
		if (InterlevelScene.mode != InterlevelScene.Mode.NONE) {
			...
			if (Dungeon.hero.hasTalent(Talent.ROGUES_FORESIGHT)
				...
				// 75%/100% chance, use level's seed so that we get the same result for the same
				// level
		...
		if (SPDSettings.intro()) {
			...
			if (inventory != null)
				inventory.visible = inventory.active = false;
		}
	...
	public void destroy() {
		...
		// tell the actor thread to finish, then wait for it to complete any actions it
		// may be doing.
	...
	// We queue any removed element destruction, rather than destroying them in the
	// actor thread.
	...
	public synchronized void update() {
		...
		if (updateTags) {
			...
			// otherwise, wait until the hero acts, so as to not suddenly change their
			// position
			if (tagAppearing)
				layoutTags();
			else
	...
	public static void layoutTags() {
		...
		// move the camera center up a bit if we're on full UI and it is taking up lots
		// of space
			...
			if (doc == Document.ADVENTURERS_GUIDE) {
				if (!page.equals(Document.GUIDE_INTRO)) {
					if (SPDSettings.interfaceSize() == 0) {
						...
						GLog.p(Messages.get(Guidebook.class, "hint_desktop", KeyBindings.getKeyName(KeyBindings
								.getFirstKeyForAction(SPDAction.JOURNAL, ControllerHandler.isControllerConnected()))));
					}
				}
		...
		if (scene != null) {
			...
			if (SPDSettings.interfaceSize() == 0) {
				...
				GLog.p(Messages.get(GameScene.class, "tutorial_ui_desktop",
						KeyBindings.getKeyName(KeyBindings.getFirstKeyForAction(SPDAction.HERO_INFO,
								ControllerHandler.isControllerConnected())),
						KeyBindings.getKeyName(KeyBindings.getFirstKeyForAction(SPDAction.INVENTORY,
								ControllerHandler.isControllerConnected()))));
			}
	...
	public static void updateKeyDisplay() {
		if (scene != null && scene.menu != null)
			scene.menu.updateKeys();
	}

	public static void showlevelUpStars() {
		if (scene != null && scene.status != null)
			scene.status.showStarParticles();
	}

	public static void updateAvatar() {
		if (scene != null && scene.status != null)
			scene.status.updateAvatar();
	}
	...
	public static void afterObserve() {
		if (scene != null) {
			for (Mob mob : Dungeon.level.mobs.toArray(new Mob[0])) {
				if (mob.sprite != null) {
					if (mob instanceof Mimic && mob.state == mob.PASSIVE && ((Mimic) mob).stealthy()
							&& Dungeon.level.visited[mob.pos]) {
	...
	public static void flash(int color, boolean lightmode) {
		if (scene != null) {
			...
			ShatteredPixelDungeon.runOnRenderThread(new Callback() {
				@Override
				public void call() {
					// greater than 0 to account for negative values (which have the first bit set
					// to 1)
					if (scene != null) {
	...
	public static String floorBossClaimMessage(int depth) {
		return Messages.get(GameScene.class, "floor_boss", floorBossTitle(depth), depth);
	}

	static String floorBossTitle(int depth) {
		return Messages.get(GameScene.class, "floor_boss_title_" + depth);
	}

	public static boolean showsFloorBossClaim(int depth, boolean heroAlive) {
		return EchoCaptureTrigger.shouldCapture(depth, heroAlive);
	}
	...
	public static WndBag selectItem(WndBag.ItemSelector listener) {
		...
		if (scene != null) {
			// TODO can the inventory pane work in these cases? bad to fallback to mobile
			// window
	...
	// logic for preserving inventory selection windows on scene reset (e.g. via
	// auto-rotate)
