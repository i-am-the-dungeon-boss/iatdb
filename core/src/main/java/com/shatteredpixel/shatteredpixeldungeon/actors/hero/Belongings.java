...
public class Belongings implements Iterable<Item> {
	...
	// used to ensure that the duelist always uses the weapon she's using the
	// ability of
	...
	// *** these accessor methods are so that worn items can be affected by various
	// effects/debuffs
	// we still want to access the raw equipped items in cases where effects should
	// be ignored though,
	// such as when equipping something, showing an interface, or dealing with items
	// from a dead hero

	// normally the primary equipped weapon, but can also be a thrown weapon or an
	// ability's weapon
	...
	// we cache whether belongings are lost to avoid lots of calls to
	// hero.buff(LostInventory.class)
	...
	public void identify() {
		// Stale Echo kit curUser must not block setKnown on potions/scrolls/rings.
		Item.clearCurrent();
		for (Item item : this) {
			item.identify(false);
		}
