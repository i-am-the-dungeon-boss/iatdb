...
public abstract class Char extends Actor {
		...
		if (enemy.isInvulnerable(getClass())) {
			...
			if (Dungeon.hero.alignment == enemy.alignment
					...
					&& (Dungeon.level.distance(enemy.pos, Dungeon.hero.pos) <= 2
							|| enemy.buff(LifeLinkSpell.LifeLinkSpellBuff.class) != null)) {
			...
			// This matters as defence procs can sometimes inflict self-damage, such as
			// armor glyphs.
			if (!enemy.isAlive()) {
				return true;
			}
			...
			Talent.CombinedLethalityAbilityTracker combinedLethality = buff(
					Talent.CombinedLethalityAbilityTracker.class);
			if (combinedLethality != null && this instanceof Hero
					&& ((Hero) this).belongings.attackingWeapon() instanceof MeleeWeapon
				...
				if (enemy.isAlive() && enemy.alignment != alignment && !Char.hasProp(enemy, Property.BOSS)
						...
						(enemy.HP / (float) enemy.HT) <= 0.4f * ((Hero) this).pointsInTalent(Talent.COMBINED_LETHALITY)
								/ 3f) {
					enemy.HP = 0;
					if (enemy.buff(Brute.BruteRage.class) != null) {
						enemy.buff(Brute.BruteRage.class).detach();
			...
			if (enemy.sprite != null) {
				if (hitMissIcon != -1) {
					...
					if (hitMissIcon == FloatingText.MISS_TUFT && Messages.lang() == Languages.ENGLISH
							&& Random.Int(10) == 0) {
			...
			if (visibleFight) {
				// TODO enemy.defenseSound? currently miss plays for monks/crab even when they
				// parry
	...
	public int defenseProc(Char enemy, int damage) {
		...
		// hero and pris images skip this as they already benefit from hero's armor
		// glyph proc
		if (!(this instanceof Hero || this instanceof PrismaticImage)) {
			if (Dungeon.hero.alignment == alignment && Dungeon.hero.belongings.armor() != null
					...
					&& (Dungeon.level.distance(pos, Dungeon.hero.pos) <= 2
							|| buff(LifeLinkSpell.LifeLinkSpellBuff.class) != null)) {
	...
	// Returns the level a glyph is at for a char, or -1 if they are not benefitting
	// from that glyph
	// This function is needed as (unlike enchantments) many glyphs trigger in a
	// variety of cases
	...
	public float speed() {
		float speed = baseSpeed * timedSpeedMultiplier();
	...
	/**
	 * Multiplier from temporary movement buffs on this character (Haste potion,
	 * Stamina, Adrenaline, Cripple, Dread). Does not include rings, armor, or
	 * glyphs.
	 */
	public float timedSpeedMultiplier() {
		return timedSpeedMultiplier(this);
	}

	/**
	 * Same as {@link #timedSpeedMultiplier()} but a buff on either character counts
	 * once. Used when Echo potion buffs live on the boss body while gear lives on
	 * the phantom kit.
	 */
	public static float timedSpeedMultiplier(Char primary, Char also) {
		float speed = 1f;
		if (hasBuff(primary, also, Cripple.class))
			speed /= 2f;
		if (hasBuff(primary, also, Stamina.class))
			speed *= 1.5f;
		if (hasBuff(primary, also, Adrenaline.class))
			speed *= 2f;
		if (hasBuff(primary, also, Haste.class))
			speed *= 3f;
		if (hasBuff(primary, also, Dread.class))
			speed *= 2f;
		return speed;
	}

	private static float timedSpeedMultiplier(Char ch) {
		return timedSpeedMultiplier(ch, null);
	}

	private static boolean hasBuff(Char a, Char b, Class<? extends Buff> cls) {
	...
	// these are misc. sources of physical damage which do not apply armor, they get
	// a different icon
	...
	// This is relevant because we call isAlive during drawing, which has both
	// performance
	...
	@SuppressWarnings("unchecked")
	// returns an instance of the specific buff class, if it exists. Not just
	// assignable
	...
	// travelling may be false when a character is moving instantaneously, such as
	// via teleportation
	...
	public HashSet<Property> properties() {
		...
		// TODO any more of these and we should make it a property of the buff, like
		// with resistances/immunities
	...
	public enum Property {
		BOSS(new HashSet<Class>(
				Arrays.asList(Grim.class, GrimTrap.class, ScrollOfRetribution.class, ScrollOfPsionicBlast.class)),
		...
		// A character that acts in an unchanging manner. immune to AI state debuffs or
		// stuns/slows
		STATIC(new HashSet<Class>(),
				new HashSet<Class>(
						Arrays.asList(AllyBuff.class, Dread.class, Terror.class, Amok.class, Charm.class, Sleep.class,
