...
public abstract class Level implements Bundlable {
	...
	public void create() {
		...
		if (!Dungeon.bossLevel() && Dungeon.branch == 0) {
			...
			if (Dungeon.souNeeded()) {
				...
				// TODO while this does significantly reduce this challenge's levelgen impact,
				// it doesn't quite remove it
				// for 0 levelgen impact, we need to do something like give the player all SOU,
				// but nerf them
				// or give a random scroll (from a separate RNG) instead of every 2nd SOU
				if (!Dungeon.isChallenged(Challenges.NO_SCROLLS)
						|| Dungeon.LimitedDrops.UPGRADE_SCROLLS.count % 2 != 0) {
	...
	public float respawnCooldown() {
		...
		if (Statistics.amuletObtained) {
			if (Dungeon.depth == 1) {
				...
				cooldown = Math.round(GameMath.gate(TIME_TO_RESPAWN / 10f,
	...
	public Heap drop(Item item, int cell) {
		...
		if (heap == null) {
			...
			// Headless / no GameScene: callers often do drop(...).sprite.drop().
			if (heap.sprite == null) {
				ItemSprite sprite = heap.sprite = new ItemSprite();
				sprite.link(heap);
			}
	...
	public void occupyCell(Char ch) {
		...
		if (!ch.flying) {
			...
			// we call act here instead of detach in case the debuffs haven't managed to
			// deal dmg once yet
	...
	public void updateFieldOfView(Char c, boolean[] fieldOfView) {
		...
		if (sighted) {
			...
			// grass is see-through by some specific entities, but not during the fungi
			// quest
			if (!(Dungeon.level instanceof MiningLevel) || Blacksmith.Quest.Type() != Blacksmith.Quest.FUNGI) {
				if ((c instanceof Hero && ((Hero) c).subClass == HeroSubClass.WARDEN)
					...
					if (blocking == null) {
						System.arraycopy(Dungeon.level.losBlocking, 0, modifiableBlocking, 0,
								modifiableBlocking.length);
				...
				if (mindVisRange >= 1) {
					for (Mob mob : mobs) {
						...
						if (!fieldOfView[p] && (distance(c.pos, p) <= mindVisRange
								|| (ally != null && distance(ally.pos, p) <= mindVisRange))) {
							for (int i : PathFinder.NEIGHBOURS9) {
								heroMindFov[mob.pos + i] = true;
							}
						}
