...
public class CavesBossLevel extends Level {
	...
	public void activatePylon() {
		...
		if (pylons.size() == 1) {
			...
			for (Pylon p : pylons) {
				if (closest == null
						|| trueDistance(p.pos, Dungeon.hero.pos) < trueDistance(closest.pos, Dungeon.hero.pos)) {
					closest = p;
				}
			}
		...
		public Tilemap create() {
			...
			for (int i = 0; i < data.length; i++) {
				...
				if (i % tileW == tileW / 2 - 2) {
					...
					// otherwise check if we are on row 2 or 3, in which case we need to override
					// walls
				} else {
					if (i / tileW == 2) {
						data[i] = 13;
					...
					if (off[cell] > 0) {
						...
						if (ch != null && !(ch instanceof DM300) && !ch.flying) {
							if (ch instanceof Mob) {
								// incredibly specific but I'll 100% get a bug report in a year if I don't add
								// this
