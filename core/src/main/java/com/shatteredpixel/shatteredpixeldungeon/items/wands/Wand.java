...
public abstract class Wand extends Item {
	...
	public int buffedLvl() {
		...
		if (charger != null && charger.target != null) {
			...
			if (curCharges == 1 && charger.target instanceof Hero
					&& ((Hero) charger.target).hasTalent(Talent.DESPERATE_POWER)) {
			...
			if (target != null) {
				...
				if (target == curUser.pos || cell == curUser.pos) {
					if (target == curUser.pos && curUser.hasTalent(Talent.SHIELD_BATTERY)) {
						...
						curUser.sprite.showStatusWithIcon(CharSprite.POSITIVE, Integer.toString(Math.round(shield)),
								FloatingText.SHIELDING);
