...
public class MagesStaff extends MeleeWeapon {
	...
	public Item imbueWand(Wand wand, Char owner) {
		...
		// FIXME this is gross, should implement a better way to fully reset quickslot
		// visuals
	...
	/** Sets imbued wand charges (clamped). No-op when no wand is imbued. */
	public void setWandCharges(int charges) {
		if (wand == null) {
			return;
		}
		wand.curCharges = Math.max(0, Math.min(wand.maxCharges, charges));
	}
	...
	public String info() {
		...
		if (wand != null) {
			...
			if ((!cursed && !hasCurseEnchant()) || !cursedKnown)
				...
				info += " " + Messages.get(this, "cursed_wand");
	...
	private final Emitter.Factory StaffParticleFactory = new Emitter.Factory() {
		@Override
		// reimplementing this is needed as instance creation of new staff particles
