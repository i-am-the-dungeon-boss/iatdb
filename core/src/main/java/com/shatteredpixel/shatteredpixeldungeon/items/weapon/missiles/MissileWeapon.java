...
abstract public class MissileWeapon extends Weapon {
	...
	// TODO maybe make this like actor IDs, instead of random? collisions unlikely,
	// but it's messy
	public long setID = new SecureRandom().nextLong();

	// whether or not this instance of the item exists purely to trigger its effect.
	// i.e. no dropping
	...
	public int throwPos(Hero user, int dst) {
		...
		if ((!(this instanceof SpiritBow.SpiritArrow)
				&& Random.Int(3) < user.pointsInTalent(Talent.SHARED_ENCHANTMENT))) {
		...
		if (projecting > 0
				...
				&& Dungeon.level.distance(user.pos, dst) <= Math
						.round(projecting * Enchantment.genericProcChanceMultiplier(user))) {
			return dst;
		} else {
	...
	// mainly used to track warnings relating to throwing the last upgraded one, not
	// super accurate
	...
	// classes that add steps onto durabilityPerUse can turn rounding off, to do
	// their own rounding after more logic
	...
	public Item merge(Item other) {
		...
		if (isSimilar(other)) {
			...
			// if other has a curse/enchant status that's a higher priority, copy it. in the
			// following order:
			// curse infused
			if (!curseInfusionBonus && ((MissileWeapon) other).curseInfusionBonus
					&& ((MissileWeapon) other).hasCurseEnchant()) {
	...
	public void restoreFromBundle(Bundle bundle) {
		...
		if (bundle.contains(SET_ID)) {
			...
			// if we have a higher than 0 level, assume that this was a solitary thrown wep
			// upgrade
			...
			if (level() > 0) {
				...
				// otherwise treat all currently spawned thrown weapons of the same class as if
