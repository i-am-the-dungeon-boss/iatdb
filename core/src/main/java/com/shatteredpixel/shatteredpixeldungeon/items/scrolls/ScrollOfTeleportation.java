...
public class ScrollOfTeleportation extends Scroll {
	...
	public static void appear(Char ch, int pos) {
		...
		if (Dungeon.level.heroFOV[ch.pos] && ch != Dungeon.hero && GameScene.emitter() != null) {
		...
		if (ch.invisible == 0 && ch.sprite != null && ch.sprite.parent != null) {
			ch.sprite.alpha(0);
			ch.sprite.parent.add(new AlphaTweener(ch.sprite, 1, 0.4f));
		}

		if (ch.sprite != null) {
			if ((Dungeon.level.heroFOV[pos] || ch == Dungeon.hero) && ch.sprite.emitter() != null) {
				ch.sprite.emitter().start(Speck.factory(Speck.LIGHT), 0.2f, 3);
			} else if (Camera.main != null && Camera.main.followTarget() == ch.sprite) {
	...
	// just plays the VFX for teleporting, without any position changes, does
	// re-press cells though
	public static void appearVFX(Char ch) {
		...
		if (Dungeon.level.heroFOV[ch.pos] && ch.sprite != null && ch.sprite.emitter() != null) {
			ch.sprite.emitter().start(Speck.factory(Speck.LIGHT), 0.2f, 3);
		}
	}
