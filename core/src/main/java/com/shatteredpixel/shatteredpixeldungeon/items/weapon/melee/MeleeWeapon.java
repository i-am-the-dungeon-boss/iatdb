...
public class MeleeWeapon extends Weapon {
	...
	public void execute(Hero hero, String action) {
		...
		if (action.equals(AC_ABILITY)) {
			...
			} else if ((Buff.affect(hero, Charger.class).charges
					+ Buff.affect(hero, Charger.class).partialCharge) < abilityChargeUse(hero, null)) {
				GLog.w(Messages.get(this, "ability_no_charge"));
			} else {

				if (targetingPrompt() == null) {
					if (abilityAs(UseContext.hero(hero), null)) {
						updateQuickslot();
					}
				} else {
					...
					GameScene.selectCell(new CellSelector.Listener() {
						...
						public void onSelect(Integer cell) {
							if (cell != null) {
								if (abilityAs(UseContext.hero(hero), cell)) {
									updateQuickslot();
								}
							}
						}
	...
	/**
	 * Shared duelist weapon ability (cell already chosen). Self-effects target
	 * {@code ctx.body}; charge/talents use {@code ctx.kit}; turn via
	 * {@code ctx.turns}.
	 */
	public boolean abilityAs(UseContext ctx, Integer target) {
		Hero kit = ctx.kit;
		if (kit == null || !isEquipped(kit)) {
			return false;
		}
		...
		if ((charger.charges + charger.partialCharge) < abilityChargeUse(kit, null)) {
			return false;
		}
		if (targetingPrompt() != null && target == null) {
			return false;
		}
		// Do not busy before validating — refused targets must leave Hero ready.
		// Successful abilities busy themselves before async VFX / spendAndNext.
		return duelistAbility(ctx, target);
	}
	...
	/**
	 * Weapon ability body. Return true when the ability ran (turn spent).
	 * Prefer {@link UseContext} overrides. Default: legacy
	 * {@link #duelistAbility(Hero, Integer)} only when {@code ctx.body == ctx.kit}.
	 */
	protected boolean duelistAbility(UseContext ctx, Integer target) {
		if (ctx.body != ctx.kit) {
			return false;
		}
		duelistAbility(ctx.kit, target);
		return true;
	}
	...
	protected void beforeAbilityUsed(UseContext ctx, Char target) {
		Hero kit = ctx.kit;
		kit.belongings.abilityWeapon = this;
		Charger charger = Buff.affect(kit, Charger.class);

		charger.partialCharge -= abilityChargeUse(kit, target);
	...
	protected void beforeAbilityUsed(Hero hero, Char target) {
		beforeAbilityUsed(UseContext.hero(hero), target);
	}

	protected void afterAbilityUsed(UseContext ctx) {
		Hero kit = ctx.kit;
		...
		if (kit.hasTalent(Talent.VARIED_CHARGE)) {
			...
			if (tracker == null || tracker.weapon == getClass() || tracker.weapon == null) {
				Buff.affect(kit, Talent.VariedChargeTracker.class).weapon = getClass();
			} else {
				tracker.detach();
				Charger charger = Buff.affect(kit, Charger.class);
		...
		if (kit.hasTalent(Talent.COMBINED_LETHALITY)) {
			...
			if (tracker == null || tracker.weapon == this || tracker.weapon == null) {
				Buff.affect(kit, Talent.CombinedLethalityAbilityTracker.class, kit.cooldown()).weapon = this;
			} else {
				tracker.detach();
			}
		}
	...
	protected void afterAbilityUsed(Hero hero) {
		afterAbilityUsed(UseContext.hero(hero));
	}
	...
	public int buffedLvl() {
		if (!evaluatingTwinUpgrades && Dungeon.hero != null && isEquipped(Dungeon.hero)
				&& Dungeon.hero.hasTalent(Talent.TWIN_UPGRADES)) {
	...
	public String info() {
		...
		if (isEquipped(Dungeon.hero) && !hasCurseEnchant() && Dungeon.hero.buff(HolyWeapon.HolyWepBuff.class) != null
			...
			info += "\n\n" + Messages.capitalize(Messages.get(Weapon.class, "enchanted",
					Messages.get(HolyWeapon.class, "ench_name", Messages.get(Enchantment.class, "enchant"))));
	...
	public static class Charger extends Buff implements ActionIndicator.Action {
		...
		public boolean act() {
			if (charges < chargeCap()) {
				if (Regeneration.regenOn()) {
					...
					if (hero != null && hero.subClass == HeroSubClass.CHAMPION) {
						chargeToGain *= 1.5f;
					}

					// 50% slower charge gain with brawler's stance enabled, even if buff is
					// inactive
					if (hero != null && hero.buff(RingOfForce.BrawlersStance.class) != null) {
						chargeToGain *= 0.50f;
					}
				...
				if (points > 0 && target.buff(Recharging.class) != null
						|| target.buff(ArtifactRecharge.class) != null) {
			...
			if (ActionIndicator.action != this && Dungeon.hero != null
					&& Dungeon.hero.subClass == HeroSubClass.CHAMPION
					&& target == Dungeon.hero) {
		...
		public void doAction() {
			...
			if (Dungeon.hero.belongings.secondWep == null
					&& Dungeon.hero.belongings.backpack.items.size() >= Dungeon.hero.belongings.backpack.capacity()) {
