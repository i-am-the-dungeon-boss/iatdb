...
public class HeroSprite extends CharSprite {
	...
	public void bloodBurstA(PointF from, int damage) {
		 ...
		 * characters which
		 * participate in combat, so removing all blood associated with them is a simple
		 * way to
	...
	/** Armor-tier spritesheet cell by frame index (run/attack/idle cycles). */
	public static Image sheetFrame(HeroClass cl, int armorTier, int frameIndex) {
		Image image = new Image(cl.spritesheet());
		TextureFilm film = new TextureFilm(tiers(), armorTier, FRAME_WIDTH, FRAME_HEIGHT);
		image.frame(film.get(frameIndex));
		return image;
	}
}
